...
def hand_type_value(hand):
    if 'J' not in hand or hand == 'JJJJJ':
        return hand_type_value_no_J(hand)
    else:
        counts = {card: hand.count(card) for card in set(hand) - set('J')}
        # print(counts, hand)
        max_shown = sorted(counts.items(), key=lambda x: x[1])[-1][0]
        hand_temp = hand.replace('J', max_shown)
        return hand_type_value_no_J(hand_temp)
